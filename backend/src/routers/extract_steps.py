...
async def extract_steps(
    ...
    start_date, end_date, months_of_extract = query_params
    ...
    if has_date_range and has_months:
        raise HTTPException(
            status_code=400,
            detail="start_date/end_date と months_of_extract は同時に指定できません。どちらか一方を指定してください。",
        )

    if not has_date_range and not has_months:
        raise HTTPException(
            status_code=400,
            detail="start_date/end_date の両方、または months_of_extract のいずれかを指定してください。",
        )
    ...
    if has_date_range:
        ...
        if start_date is None or end_date is None:
            raise HTTPException(
                status_code=400,
                detail="start_date と end_date は両方指定する必要があります。",
            )
